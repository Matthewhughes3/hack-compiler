...
endsWithEquals s = let (e:_) = drop (length s - 1) s 
                   in  e=='='

cDest :: Parser String
cDest = compileCDest <$> ((do
    d <- some $ satisfy (/='=')
    satisfy (=='=')
    return d) <|> return [])

cComp :: Parser String
cComp = compileCComp <$> (do
    some $ satisfy (/=';'))
...
cInstruction = compileCInstruction <$> (do
    dest <- cDest
    comp <- cComp
    jmp <- cJmp
